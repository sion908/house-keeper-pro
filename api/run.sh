...
if [ $# = 0 ]; then
  ...
  python -m pip install -r requirements.txt -t /api/python_modules/python/lib/python3.10/site-packages/ --upgrade --verbose
  echo installed lib
  if [ ! -e python ]; then
    mkdir python
  fi
  ...
  poetry install
  poetry run uvicorn main:app --reload --log-level debug --host 0.0.0.0 --port 80
